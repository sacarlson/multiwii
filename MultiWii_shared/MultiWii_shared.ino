...
void annexCode() { // this code is excetuted at each loop and won't interfere with control loop if it lasts less than 650 microseconds
  ...
  for(axis=0;axis<3;axis++) {
    ...
    //scotty's  experiment to enable adjustment of P gain in flight with AUX4 nob channel of TH9X r/c radio adjustable from 1 to 10
    dynP8[axis] = ((uint16_t)conf.P8[axis]*prop1/100)*(1+(rcData[AUX4]-1045)/100);
 ...
 //*********************************** 
  ...
  for(axis=0;axis<3;axis++) {
    ...
    if ( !f.ANGLE_MODE || f.HORIZON_MODE || axis == 2 ) { // MODE relying on GYRO or YAW axis
      // scotty's expermental P gain adjustable with AUX4 nob of TH9X r/c radio, value changeable from 1 to 10
      if (abs(rcCommand[axis])<350) error =          (rcCommand[axis]*10*8/conf.P8[axis])*(1+(rcData[AUX4]-1045)/100) ; // 16 bits is needed for calculation: 350*10*8 = 28000      16 bits is ok for result if P8>2 (P>0.2)
                               else error = ((int32_t)rcCommand[axis]*10*8/conf.P8[axis])*(1+(rcData[AUX4]-1045)/100) ; // 32 bits is needed for calculation: 500*5*10*8 = 200000   16 bits is ok for result if P8>2 (P>0.2)
      error -= gyroData[axis];
